...
def validate_setting(key, value):
    if key == "font_size":
        try:
            ...
            print("Font size must be a nuber.")
            return False, None

    if key == "theme":
        allowed_thme = ["orange", "blue", "green"]
        cleaned_value = value.strip().lower()
        
        if cleaned_value in allowed_thme:
            return True, cleaned_value
        else:
            print(f"Please enter from the following options: {', '.join(allowed_thme)}.")
            return False, None
...
def main():
    ...
    while True:
        ...
        target_setting = input("Enter the setting you want to modify (or 'exit' to quit):")
        ...
        if is_valid:
            ...
            print(f"Invalid value. {validated_value} Please try again.")
